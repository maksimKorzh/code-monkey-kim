...
/* VTL-02 interpreter */
/* for use, see VTL-02 on github */
#ifdef AVRX
const unsigned char vtl[1019] PROGMEM = {  
#else
unsigned char vtl[1019] = {
#endif
...
// inserted (hacked in) for VTL-02: CFF5 is CFF4 without Microchess key translations
...
// --------------------------------
  ...
  if ((address >= 0xFC00) && (address <= 0xFFFA))   // VTL-02
  { 
    #ifdef AVRX
    return(pgm_read_byte_near(vtl + address - 0xFC00)); // VTL-02 ROM
    #else
    return calcRom[vtl - 0x5000];
    #endif
  }
